...
{
    //While the current character is not null, continue. strings are stored in memory like this -> 'H''E''L''L''O''\0'
    //so we should send the charcaters one by one until  we reach '\0'
    while (*str)
    {
        USART2_SendChar(*str); //sends the current character
        str++; // go to the next character. The pointer advances by one byte.
    }
}
